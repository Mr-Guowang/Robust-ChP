...
class nnUNetTrainer(object):
    ...
    def get_dataloaders(self):
        ...
        tr_transforms = self.get_training_transforms(
            ...
            ignore_label=self.label_manager.ignore_label,first_n_channels = self.first_n_channels)
    ...
    def on_epoch_end(self):
        ...
        # if self._best_ema is None or self.logger.my_fantastic_logging['dice_per_class_or_region'][-1] > self._best_ema:
        #     self._best_ema = self.logger.my_fantastic_logging['dice_per_class_or_region'][-1]
        #     self.print_to_log_file(f"Yayy! New best EMA pseudo Dice: {np.round(self._best_ema, decimals=4)}")
        #     self.save_checkpoint(join(self.output_folder, 'checkpoint_best.pth'))
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            with torch.no_grad():
                ...
                # val_outputs = self.Actual_validation()
                # self.on_validation_epoch_end_per(val_outputs)
            self.on_epoch_end()
        self.on_train_end()

    def Actual_validation(self, save_probabilities: bool = False):
        self.set_deep_supervision_enabled(False)
        self.network.eval()
        if self.is_ddp and self.batch_size == 1 and self.enable_deep_supervision and self._do_i_compile():
            self.print_to_log_file("WARNING! batch size is 1 during training and torch.compile is enabled. If you "
                                   ...
                                   "forward pass (where compile is triggered) already has deep supervision disabled. "
                                   "This is exactly what we need in perform_actual_validation")
        predictor = nnUNetPredictor(tile_step_size=0.25, use_gaussian=True, use_mirroring=False,
                                    perform_everything_on_device=True, device=self.device, verbose=False,
                                    verbose_preprocessing=False, allow_tqdm=False)
        predictor.manual_initialization(self.network, self.plans_manager, self.configuration_manager, None,
                                        self.dataset_json, self.__class__.__name__,
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.identifiers):
                # self.print_to_log_file(f"predicting {k}")
                data, target, _, _ = dataset_val.load_case(k)
                # we do [:] to convert blosc2 to numpy
                data = data[:]
                target = target[:]
                target[target==-1] = 0
                ...
                _, D, H, W = data.shape
                dz, dy, dx = 96, 288, 250   # 96, 288, 250   128,128,128
                coords = torch.nonzero(target.sum(0) > 0, as_tuple=False)
                if coords.numel() == 0:  # 没有前景 -> 取中心
                    cz, cy, cx = D // 2, H // 2, W // 2
                else:
                    cz, cy, cx = coords.float().mean(dim=0).long()
                # 计算起止索引
                ...
                # 如果超出边界，先裁，再 pad
                pad_z1, pad_y1, pad_x1 = max(0, -z1), max(0, -y1), max(0, -x1)
                pad_z2, pad_y2, pad_x2 = max(0, z2 - D), max(0, y2 - H), max(0, x2 - W)

                z1, z2 = max(z1, 0), min(z2, D)
                y1, y2 = max(y1, 0), min(y2, H)
                x1, x2 = max(x1, 0), min(x2, W)

                data_patch = data[:, z1:z2, y1:y2, x1:x2]
                target_patch = target[:, z1:z2, y1:y2, x1:x2]
                # pad 成 (dz,dy,dx)
                padding = (pad_x1, pad_x2, pad_y1, pad_y2, pad_z1, pad_z2)
                data = F.pad(data_patch, padding)
                target = F.pad(target_patch, padding)
              
                # self.print_to_log_file(f'{k}, shape {data.shape}, rank {self.local_rank}')
                output_filename_truncated = join(validation_output_folder, k)
                prediction = predictor.predict_sliding_window_return_logits(data)
                prediction = prediction.cpu().unsqueeze(0)
                output_seg = prediction.argmax(1)[:, None]
                predicted_segmentation_onehot = torch.zeros(prediction.shape, device=prediction.device, dtype=torch.float32)
                predicted_segmentation_onehot.scatter_(1, output_seg, 1)
                del output_seg
                tp, fp, fn, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot, target.unsqueeze(0), axes=[0, 2, 3, 4], mask=None)
                tp_hard = tp.detach().cpu().numpy()
                fp_hard = fp.detach().cpu().numpy()
                fn_hard = fn.detach().cpu().numpy()
                if not self.label_manager.has_regions:
                    ...
                    fp_hard = fp_hard[1:]
                    fn_hard = fn_hard[1:]
                results.append({'loss': torch.tensor([[0]]).detach().cpu().numpy(), 'tp_hard': tp_hard, 'fp_hard': fp_hard, 'fn_hard': fn_hard})
        self.set_deep_supervision_enabled(True)
        compute_gaussian.cache_clear()
        return results
    
    def on_validation_epoch_end_per(self, val_outputs: List[dict]):
        outputs_collated = collate_outputs(val_outputs)
        per_case_dice = []
        for tp_case, fp_case, fn_case in zip(outputs_collated['tp_hard'],
                                            outputs_collated['fp_hard'],
                                            outputs_collated['fn_hard']):
            dice_case = [2 * i / (2 * i + j + k) if (2 * i + j + k) > 0 else np.nan
                        for i, j, k in zip(tp_case, fp_case, fn_case)]
            per_case_dice.append(dice_case)
        global_dc_per_class = np.nanmean(per_case_dice, axis=0)
